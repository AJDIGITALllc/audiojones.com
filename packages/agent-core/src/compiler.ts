...
import { Plan, Action, Platform } from './dsl/v1/types.js';
 ...
 */
export async function compilePlanFromIntent(input: PlanInput): Promise<Plan> {
  const planId = generatePlanId();
  const createdAt = new Date().toISOString();
  
  // Parse the prompt to determine actions
  const actions = parsePromptToActions(input.prompt, input.context?.platform as Platform);
 ...
 */
function parsePromptToActions(prompt: string, defaultPlatform: Platform = 'whop'): Action[] {

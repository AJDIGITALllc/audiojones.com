...
export default function PaymentsPage() {
  ...
  if (!user) return <div className="text-white/80">Please sign in to view your payments.</div>;
        ...
        <div className="divide-y divide-white/10 rounded-xl border border-white/10 bg-white/5">
          {items.map((p) => (
            <div key={p.id} className="grid grid-cols-3 gap-4 p-4">
              ...
              <div className="text-white/80">{p.amount ? (p.amount / 100).toFixed(2) : "-"} {p.currency?.toUpperCase()}</div>
